...
const SVCS = {
  ...
  togo_expertise: {
    name:"Consultances/Expertises", icon:"🧑‍💼", bg:"#EDE7F6", active:false, soon:"07 Avril 2026",
    fields:[]
  },
  mathivick: {
...
// ════════════════════════════════════════════════════════════════
// CONSULTANCES/EXPERTISES — Navigation Mathivick & Omega Conseils
// ════════════════════════════════════════════════════════════════

// Données des services Mathivick (3 grandes lignes + sous-options)
const MATHIVICK_SERVICES = [
  {
    id: 'math_formation',
    label: '1. Formation et développement des compétences',
    emoji: '🎓',
    desc: 'Formations pratiques et interactives pour générer des résultats rapides et durables',
    options: [
      'Initiation au métier du commercial',
      'Techniques avancées de vente',
      'Prospection commerciale efficace',
      'Relation client & fidélisation',
      'Communication digitale pour les ventes',
      'Gestion et planification commerciale',
    ]
  },
  {
    id: 'math_marketing',
    label: '2. Service marketing & commercial',
    emoji: '📣',
    desc: 'Parce que bien vendre passe par bien se faire connaître',
    options: [
      'Stratégie commerciale (communication & vente)',
      'Création de contenus (visuels, vidéos, posts)',
  ...
  {
    id: 'math_produits',
    label: '3. Commercialisation de produits frais & bios',
    emoji: '🥬',
    desc: 'Accompagnement à la vente de produits frais et biologiques de qualité',
    options: [
      'Produits agricoles maraîchers',
      'Produits saisonniers de qualité',
      'Produits issus d\'aviculture (volailles) et de pisciculture (poissons)',
      'Produits sans additifs chimiques',
      'Orientation santé, bien-être et alimentation consciente',
    ]
  },
];

// Données des services Omega Conseils (3 grandes lignes + sous-options)
const OMEGA_SERVICES = [
  {
    id: 'omega_redaction',
    label: '1. Rédaction & Montage de projets',
    emoji: '📄',
    ...
    options: [
      ...
      'Montage de dossiers pour bailleurs (Banque mondiale, BAD, BOAD...)',
      'Élaboration de notes conceptuelles & projets PPP',
      'Modélisation financière complète (VAN, TRI, cash-flow)',
      'Élaboration de plans stratégiques (3 à 5 ans)',
    ]
  },
  {
    id: 'omega_entrepreneuriat',
    label: '2. Entrepreneuriat & Création d\'entreprise',
    emoji: '🚀',
    desc: 'Accompagnement complet de la création à la structuration de votre entreprise',
    options: [
      'Accompagnement à la création juridique d\'entreprise',
      'Étude de marché et validation du modèle économique',
      'Structuration du business model (Canvas)',
      'Formalisation administrative et fiscale',
      'Stratégie de lancement et positionnement commercial',
    ]
  },
  {
    id: 'omega_formation',
    label: '3. Formation professionnelle et renforcement de capacités',
    emoji: '🎓',
    desc: 'Formations spécialisées en montage de projets, finance et entrepreneuriat',
    options: [
      'Formation en montage de projets',
      'Formation en finance structurée',
      'Formation en gestion des risques',
      'Formation en entrepreneuriat pratique',
      'Ateliers de rédaction de business plan',
    ]
  },
];

// ── Niveau 1 : menu principal Consultances/Expertises ──
function showTogoExpertiseMenu() {
  ...
  overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;overflow-y:auto;";
  overlay.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:24px 22px;max-width:400px;width:100%;box-shadow:0 8px 40px rgba(0,0,0,0.2);">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;">
        <div style="width:48px;height:48px;background:#EDE7F6;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:26px;flex-shrink:0;">🧑‍💼</div>
        <div style="flex:1">
          <div style="font-weight:700;font-size:17px;color:#1A1A2E;">Consultances/Expertises</div>
          <div style="font-size:12px;color:#9E9EC0;">Choisissez un service</div>
        </div>
        <button onclick="document.getElementById('togo-expertise-overlay').remove()" style="background:none;border:none;font-size:22px;color:#9E9EC0;cursor:pointer;padding:4px;">×</button>
      </div>
      <p style="color:#666;font-size:13px;margin:10px 0 18px;line-height:1.5;">Deux pôles complémentaires pour accompagner vos projets et développer vos compétences.</p>
      <div style="display:flex;flex-direction:column;gap:10px;">
        <button onclick="showConsultancesSubMenu('mathivick')" style="width:100%;display:flex;align-items:center;gap:14px;padding:16px;background:#FFF3E0;border:1.5px solid #FFE0B2;border-radius:14px;cursor:pointer;text-align:left;transition:all .15s;" onmouseover="this.style.background='#FFE0B2'" onmouseout="this.style.background='#FFF3E0'">
          <div style="width:46px;height:46px;background:#fff;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:24px;box-shadow:0 2px 8px rgba(0,0,0,0.08);flex-shrink:0;">🚀</div>
          <div style="flex:1">
            <div style="font-weight:700;font-size:15px;color:#1A1A2E;">Mathivick</div>
            <div style="font-size:12px;color:#888;margin-top:2px;">Formation commerciale & marketing</div>
          </div>
          <span style="font-size:18px;color:#D0D0E0;">›</span>
        </button>
        <button onclick="showConsultancesSubMenu('omega_conseil')" style="width:100%;display:flex;align-items:center;gap:14px;padding:16px;background:#E3F2FD;border:1.5px solid #BBDEFB;border-radius:14px;cursor:pointer;text-align:left;transition:all .15s;" onmouseover="this.style.background='#BBDEFB'" onmouseout="this.style.background='#E3F2FD'">
          <div style="width:46px;height:46px;background:#fff;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:24px;box-shadow:0 2px 8px rgba(0,0,0,0.08);flex-shrink:0;">💼</div>
          <div style="flex:1">
            <div style="font-weight:700;font-size:15px;color:#1A1A2E;">Omega Conseils</div>
            <div style="font-size:12px;color:#888;margin-top:2px;">Consultances & Financement de projets</div>
          </div>
          <span style="font-size:18px;color:#D0D0E0;">›</span>
        </button>
      </div>
...
// ── Niveau 2 : grandes lignes d'un service (Mathivick ou Omega) ──
function showConsultancesSubMenu(serviceId) {
  let existing = document.getElementById("togo-expertise-overlay");
  if (existing) existing.remove();

  const isMath = serviceId === 'mathivick';
  const services = isMath ? MATHIVICK_SERVICES : OMEGA_SERVICES;
  const title    = isMath ? 'Mathivick' : 'Omega Conseils';
  const subtitle = isMath ? 'Choisissez un domaine' : 'Choisissez une prestation';
  const ico      = isMath ? '🚀' : '💼';
  const bg       = isMath ? '#FFF3E0' : '#E3F2FD';
  const hoverBg  = isMath ? '#FFE0B2' : '#BBDEFB';
  const border   = isMath ? '#FFE0B2' : '#BBDEFB';
  ...
  overlay.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:24px 20px;max-width:420px;width:100%;box-shadow:0 8px 40px rgba(0,0,0,0.2);max-height:90vh;overflow-y:auto;">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;">
        <button onclick="showTogoExpertiseMenu()" style="background:#F4F6FA;border:none;width:36px;height:36px;border-radius:10px;cursor:pointer;font-size:18px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">←</button>
        <div style="width:42px;height:42px;background:${bg};border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;">${ico}</div>
        <div style="flex:1">
          <div style="font-weight:700;font-size:16px;color:#1A1A2E;">${title}</div>
          <div style="font-size:11px;color:#9E9EC0;">${subtitle}</div>
        </div>
        <button onclick="document.getElementById('togo-expertise-overlay').remove()" style="background:none;border:none;font-size:22px;color:#9E9EC0;cursor:pointer;padding:4px;">×</button>
      </div>
      <div style="display:flex;flex-direction:column;gap:10px;">
        ${services.map(s => `
          <button onclick="showConsultancesForm('${serviceId}','${s.id}')" style="width:100%;display:flex;align-items:flex-start;gap:14px;padding:14px 16px;background:${bg};border:1.5px solid ${border};border-radius:14px;cursor:pointer;text-align:left;transition:all .15s;" onmouseover="this.style.background='${hoverBg}'" onmouseout="this.style.background='${bg}'">
            <div style="width:44px;height:44px;background:#fff;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px;box-shadow:0 2px 8px rgba(0,0,0,0.08);flex-shrink:0;">${s.emoji}</div>
            <div style="flex:1">
              <div style="font-weight:700;font-size:13px;color:#1A1A2E;line-height:1.4;">${s.label}</div>
              <div style="font-size:11px;color:#9999BB;margin-top:3px;line-height:1.4;">${s.desc}</div>
            </div>
            <span style="font-size:18px;color:#D0D0E0;flex-shrink:0;margin-top:2px;">›</span>
          </button>`).join('')}
      </div>
    </div>`;
  document.body.appendChild(overlay);
...
window.showConsultancesSubMenu = showConsultancesSubMenu;

// ── Niveau 3 : formulaire de demande (style Gestion Locative, sans Budget) ──
function showConsultancesForm(serviceId, sectionId) {
  const isMath   = serviceId === 'mathivick';
  const services = isMath ? MATHIVICK_SERVICES : OMEGA_SERVICES;
  const section  = services.find(s => s.id === sectionId);
  if (!section) return;

  // Fermer l'overlay de navigation
  const existing = document.getElementById("togo-expertise-overlay");
  if (existing) existing.remove();
  ...
  // Préparer le currentImmoOption pour réutiliser submitImmoForm/submitConsultancesForm
  window._consultancesOption = {
    serviceId,
    sectionId,
  ...
  if (icoEl)    { icoEl.textContent = section.emoji; icoEl.style.background = bg; }
  ...
  if (nameEl)   nameEl.textContent = section.label;
  if (descEl)   descEl.textContent = section.desc;
  if (soonEl)   soonEl.style.display = 'block'; // bientôt disponible
  _immoFormBackFn = () => showConsultancesSubMenu(serviceId);
  if (backBtn)  backBtn.onclick = () => showConsultancesSubMenu(serviceId);

  // Rediriger le bouton submit vers submitConsultancesForm
  if (submitBtn) {
  ...
  // Reconstruire les champs du formulaire via immo-form-fields
  ...
  if (fieldsEl) {
    ...
    fieldsEl.innerHTML = `
      ...
      <select class="f-select" id="consult-ff-type">
  ...
  // Masquer seulement le budget, garder la localisation visible
...
window.showConsultancesForm = showConsultancesForm;
...
async function submitConsultancesForm() {
  ...
  // Vérifier sélection type
  const sel    = document.getElementById('consult-ff-type');
  const typeVal = sel ? sel.value : '';
  const typeLabel = sel && typeVal !== '' ? sel.options[sel.selectedIndex]?.text : '';
  if (!typeVal) {
  ...
  try {
    const docRef = await addDoc(collection(db,'commandes'), {
      ...
      serviceName:   `${opt.serviceTitle} — ${opt.sectionLabel} — ${typeLabel}`,
      statut:        'En attente',
      categorieId:   opt.sectionId,
      categorieNom:  opt.sectionLabel,
      prestation:    typeLabel,
      prestationEmoji: opt.sectionEmoji || '🧑‍💼',
      besoin,
      localisation,
      dateSouhaitee: date  || '',
      notes:         notes || '',
