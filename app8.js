...
import {
  ...
  getDocs, orderBy, serverTimestamp, doc, setDoc, getDoc, onSnapshot, deleteDoc
...
const DEFAULT_ARTICLES = {
  ...
  marketplace: [
    {id:'mk1',name:'Articles ménagers',desc:'Ustensiles, vaisselle, électroménager',price:0,unit:'Sur devis',emoji:'🏠'},
    {id:'mk2',name:'Fournitures de bureau',desc:'Papeterie, classeurs, stylos, cartouches',price:0,unit:'Sur devis',emoji:'✏️'},
    {id:'mk3',name:'Produits hygiène & beauté',desc:'Savons, shampooings, produits de soin',price:0,unit:'Sur devis',emoji:'🧴'},
    {id:'mk4',name:'Électronique & Accessoires',desc:'Câbles, batteries, accessoires informatiques',price:0,unit:'Sur devis',emoji:'📱'},
    {id:'mk5',name:'Jouets & Articles enfants',desc:'Jouets, fournitures scolaires, articles bébé',price:0,unit:'Sur devis',emoji:'🧸'},
    {id:'mk6',name:'Article personnalisé',desc:'Dites-nous ce que vous cherchez, on le trouve !',price:0,unit:'Sur devis',emoji:'🛒'},
  ]
};
...
async function confirmOrder() {
  ...
  try {
    ...
    // Bouton "Voir mes commandes" injecté dans la vue success
    const succBtn = document.getElementById('succ-view-orders-btn');
    if (succBtn) succBtn.style.display = 'block';
    showView('success');
    cart = {};
    updateCartBar();
    // Rediriger vers l'onglet commandes après 2.5s
    setTimeout(() => { goTab('orders'); }, 2500);
  } catch(err) {
    console.error(err);
...
async function submitImmoForm() {
  ...
  if (!besoin || !localisation) {
    // Highlight les champs obligatoires
    ...
    if (!localisation && locEl)    locEl.style.borderColor = '#F5820A';
...
async function _doDeleteOrder(orderId) {
  try {
    ...
    showToast('🗑️ Commande annulée et supprimée.', '#4A4A6A');
    loadMyOrders();
  } catch(e) {
    console.error(e);
    showToast('❌ Erreur lors de la suppression.', '#C62828');
  }
}
