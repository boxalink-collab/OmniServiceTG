...
// ════════════════════════════════════════
// BANDEAU PUBLICITAIRE — CAROUSEL INFINI (transform, sans scrollTo)
// Principe : on duplique toutes les cartes N fois pour créer un tunnel
// et on avance toujours vers la droite via translateX.
// Quand on a parcouru un jeu complet, on recule silencieusement d'un jeu
// (sans transition) : l'utilisateur ne voit rien.
// ════════════════════════════════════════
(function initAdbandLoop() {
  ...
  const COPIES   = 3;       // nombre de fois où on duplique le jeu de cartes
  const INTERVAL = 3500;    // ms entre deux glissements automatiques
  const DURATION = 500;     // ms de la transition CSS

  let cards    = [];        // toutes les cartes (originales × COPIES)
  let realCount = 0;        // nombre de cartes originales
  let cardW    = 0;         // largeur d'une carte + gap en px
  let pos      = 0;         // index absolu dans le tableau étendu (toujours croissant)
  let autoTimer = null;
  let isPaused  = false;

  // ── Dots ──
  function getDots() {
    return dotsContainer ? Array.from(dotsContainer.querySelectorAll('.adband-dot')) : [];
  }
  function updateDot(realIdx) {
    getDots().forEach((d, i) => d.classList.toggle('on', i === realIdx));
  }

  // ── Mesure la largeur d'une carte (avec gap) ──
  function measureCard() {
    if (!cards.length) return 0;
    const c0 = cards[0];
    const c1 = cards[1] || cards[0];
    return c1.getBoundingClientRect().left - c0.getBoundingClientRect().left || c0.offsetWidth + 16;
  }

  // ── Déplace le track vers la position `idx` (index absolu) ──
  function moveTo(idx, animate) {
    track.style.transition = animate ? `transform ${DURATION}ms cubic-bezier(0.25,0.46,0.45,0.94)` : 'none';
  ...
  function next() {
    pos++;
    moveTo(pos, true);
    updateDot(pos % realCount);

    // Après la transition, si on a dépassé le 1er jeu complet,
    // on recule silencieusement d'un jeu (sans animation).
    setTimeout(() => {
      if (pos >= realCount * (COPIES - 1)) {
        pos -= realCount;
        moveTo(pos, false);
      }
    }, DURATION + 20);
  ...
  function startLoop() {
    ...
    autoTimer = setInterval(() => { if (!isPaused) next(); }, INTERVAL);
  }
  ...
  track.addEventListener('touchend', e => {
    ...
    // Reprendre la boucle avec un délai pour laisser respirer
    setTimeout(() => { isPaused = false; }, INTERVAL);
  }, { passive: true });

  // ── Initialisation (après que Firestore ait injecté les cartes) ──
  setTimeout(() => {
    const originals = Array.from(track.querySelectorAll('.adcard'));
    realCount = originals.length;
    if (realCount === 0) return;
    if (realCount === 1) { updateDot(0); return; }

    // Vider le track et reconstruire avec COPIES exemplaires
    track.innerHTML = '';
    for (let c = 0; c < COPIES; c++) {
      originals.forEach(orig => {
        const clone = orig.cloneNode(true);
        if (c > 0) clone.classList.add('adcard-clone');
        track.appendChild(clone);
      });
    }

    // Récupérer toutes les cartes et mesurer
    cards = Array.from(track.querySelectorAll('.adcard'));

    // Désactiver le scroll natif (on pilote tout via transform)
    track.style.overflowX  = 'hidden';
    track.style.display    = 'flex';
    track.style.willChange = 'transform';

    // Petit délai pour laisser le browser calculer les positions
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        cardW = measureCard();
        // Démarrer au milieu (jeu 1 sur COPIES) pour pouvoir aussi aller à gauche
        pos = 0;
        moveTo(pos, false);
        updateDot(0);
        startLoop();
