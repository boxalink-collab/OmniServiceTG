...
// ════════════════════════════════════════
function hideSplash() {
  // Utiliser le fallback inline si disponible (plus fiable)
  if (typeof window._hideSplashFallback === 'function') {
    window._hideSplashFallback();
    return;
  }
...
// Déclencher dès que ce module s'exécute (DOM déjà prêt avec type="module")
...
function selectGestionBien(bienId, bienLabel, bienEmoji) {
  ...
  const kit = DEFAULT_KITS.find(k => k.id === 'immo3');
  if (!kit) return;
...
function selectAssistancePrestation(articleIdx, prestationLabel, prestationEmoji) {
  ...
  if (overlay) overlay.remove();
  const kit = DEFAULT_KITS.find(k => k.id === 'immo4');
  if (!kit) return;
  const article = kit.articles?.[articleIdx] || { name: prestationLabel, emoji: prestationEmoji };
  currentImmoCategory = kit;
