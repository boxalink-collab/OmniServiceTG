/* ══════════════════════════════════════════
   ...
   Firebase Firestore + Auth intégré
   ══════════════════════════════════════════ */
...
// ── ⚠️  Collez votre config Firebase ici ──
...
const SVCS = {
  ...
  delivery: {
    ...
    fields: [
      ...
      { n: "detail",  l: "Lieu de collecte / Liste",       t: "textarea",ph: "Adresse ou liste d'articles..." },
      { n: "adresse", l: "Adresse de livraison",           t: "text",    ph: "Votre adresse à Lomé" },
...
function openForm(id) {
  ...
  let html = `<div class="form-hdr">
    ...
    <div style="width:38px;height:38px;background:${svc.bg};border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px">${svc.icon}</div>
    <div>
      <div style="font-size:14px;font-weight:700;color:#1A1A2E">${svc.name}</div>
  ...
  svc.fields.forEach(f => {
    html += `<label class="f-label">${f.l}${f.opt ? ' <span style="font-weight:400;text-transform:none;color:#C5C5D8">(optionnel)</span>' : ''}</label>`;
    if (f.t === "textarea") {
      html += `<textarea class="f-textarea" rows="3" placeholder="${f.ph||''}" id="f-${id}-${f.n}"></textarea>`;
    } else if (f.t === "select") {
      html += `<select class="f-select" id="f-${id}-${f.n}"><option value="">— Choisir —</option>`;
      f.opts.forEach(o => html += `<option value="${o}">${o}</option>`);
      html += `</select>`;
    } else {
      html += `<input type="${f.t}" class="f-input" placeholder="${f.ph||''}" id="f-${id}-${f.n}"/>`;
    }
  });
...
function saveProfile() {
  ...
  setTimeout(() => btn.textContent = "Enregistrer", 2200);
