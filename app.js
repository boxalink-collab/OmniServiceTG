...
// ════════════════════════════════════════
// Flag pour éviter que onAuthStateChanged interfère pendant inscription/connexion manuelle
let _authHandledManually = false;

onAuthStateChanged(auth, async (user) => {
  // Si doLogin/doSignup gèrent eux-mêmes le currentUser, on skip
  if (_authHandledManually) return;

  if (user) {
    try {
      // Retry jusqu'à 3 fois si le profil Firestore n'est pas encore écrit
      let snap = null;
      for (let i = 0; i < 3; i++) {
...
async function doLogin() {
  ...
  try {
    ...
    // Connexion anonyme Firebase pour avoir un uid de session
    const cred = await signInAnonymously(auth);

    // Lier l'uid Firebase au profil Firestore existant
...
async function doSignup() {
  ...
  try {
    ...
    // Connexion anonyme Firebase pour générer un uid
    const cred = await signInAnonymously(auth);
    ...
    await setDoc(doc(db, 'users', cred.user.uid), profil);

    // Mettre à jour currentUser immédiatement (sans attendre Firestore)
    currentUser = { uid: cred.user.uid, nom, prenom, genre, phone: phoneNorm, ville };
...
function openService(id) {
  ...
  // Compte fantôme sans profil complet
  if (!currentUser.phone || !currentUser.nom) {
    showToast('⚠️ Inscrivez-vous pour passer une commande', '#F5820A');
...
async function loadCatalogue(svcId) {
  ...
  try {
    const q = query(collection(db,'articles'), where('service','==',svcId));
    const snap = await getDocs(q);
    snap.forEach(d => articles.push({id:d.id,...d.data()}));
    // Tri par ordre côté client
    articles.sort((a,b) => (a.ordre||99) - (b.ordre||99));
    console.log(`[Catalogue] ${articles.length} article(s) Firestore pour "${svcId}"`);
  } catch(e) {
    console.warn('[Catalogue] Firestore erreur:', e.message);
    firestoreError = true;
  }

  // Fallback uniquement en cas d'erreur Firestore (pas si la collection est simplement vide)
  if (firestoreError && articles.length === 0) articles = DEFAULT_ARTICLES[svcId] || [];
...
async function confirmOrder() {
  if (!currentUser) { openAuthModal('login'); return; }
  // Bloquer si profil incomplet (compte fantôme sans inscription)
  if (!currentUser.phone || !currentUser.nom) {
    showToast('⚠️ Complétez votre profil avant de commander', '#F5820A');
    openAuthModal('signup');
    return;
  }
...
// SLIDER HERO
// ════════════════════════════════════════
// Slider remplacé par galerie de photos (gérée via admin → photos_accueil)

async function loadHomePhotos() {
  const gallery = document.getElementById('home-gallery');
  if (!gallery) return;
  try {
    const q = query(collection(db, 'photos_accueil'));
    const snap = await getDocs(q);
    if (snap.empty) { gallery.style.display = 'none'; return; }
    const photos = [];
    snap.forEach(d => photos.push({id:d.id,...d.data()}));
    photos.sort((a,b) => (a.ordre||99) - (b.ordre||99));
    gallery.innerHTML = photos.map(p => `
      <div class="home-photo-item" ${p.lien?`onclick="window.open('${p.lien}','_blank')" style="cursor:pointer"`:''}>
        <img src="${p.imageUrl}" alt="${p.titre||''}" loading="lazy"
             onerror="this.parentElement.style.display='none'"/>
        ${p.titre ? `<div class="home-photo-caption">${p.titre}</div>` : ''}
      </div>`).join('');
    gallery.style.display = 'grid';
  } catch(e) {
    console.warn('[Photos accueil] erreur:', e.message);
