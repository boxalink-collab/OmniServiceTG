...
// ── Garantir un token Firebase Auth anonyme avant toute requête Firestore ──
// Les règles Firestore exigent request.auth != null pour lire/écrire /users
let _firebaseAuthReady = false;
const _authReadyPromise = new Promise((resolve) => {
  // onAuthStateChanged se déclenche dès qu'un utilisateur est connu (même anonyme)
  ...
  // Si pas d'utilisateur dans les 2s, forcer signInAnonymously
  setTimeout(async () => {
    if (!_firebaseAuthReady) {
      try {
        ...
        console.warn('[Auth] signInAnonymously échoué :', e.message);
        resolve(null);
      }
    }
  }, 2000);
...
async function ensureFirebaseAuth() {
  ...
  try {
    ...
    console.warn('[Auth] ensureFirebaseAuth échoué :', e.message);
    return null;
  }
}

// Démarrer la connexion anonyme immédiatement au chargement
ensureFirebaseAuth();
...
async function restoreSession() {
  ...
  // S'assurer que Firebase Auth est prêt avant de lire Firestore
...
async function doLogin() {
  ...
  try {
    // Garantir un token Firebase Auth avant la requête Firestore
    await ensureFirebaseAuth();
...
async function doSignup() {
  ...
  try {
    ...
    if (!firebaseUser) {
      err.textContent = '❌ Impossible de se connecter à Firebase. Vérifiez votre connexion.';
      return;
    }
    ...
    // Utiliser l'UID Firebase Auth anonyme — OBLIGATOIRE pour satisfaire
    // la règle : allow create: if request.auth != null && request.auth.uid == uid
