...
async function restoreSession() {
  ...
  try {
    // S'assurer que Firebase Auth a un utilisateur actif (session anonyme)
    // Si l'utilisateur Firebase Auth n'est plus connecté, se reconnecter anonymement
    if (!auth.currentUser) {
      await signInAnonymously(auth);
      // L'UID Firebase Auth anonyme peut différer de l'UID Firestore stocké —
      // c'est normal : on utilise l'UID Firestore comme identifiant profil.
    }
...
function switchAuthTab(tab) {
  ...
  const loginErr  = document.getElementById('auth-err');
...
async function doLogin() {
  ...
  try {
    // 1. Obtenir une session Firebase Auth anonyme (nécessaire pour les règles Firestore)
    if (!auth.currentUser) {
      await signInAnonymously(auth);
    }

    // 2. Chercher le compte dans Firestore par numéro de téléphone
...
async function doSignup() {
  ...
  try {
    // 1. Obtenir une session Firebase Auth anonyme
    if (!auth.currentUser) {
      await signInAnonymously(auth);
    }

    // 2. Vérifier si le numéro est déjà utilisé
    const q = query(collection(db, 'users'), where('phone', '==', phoneNorm));
    ...
    // 3. Utiliser l'UID Firebase Auth (anonyme) comme UID du profil Firestore
    //    → request.auth.uid == uid dans les règles Firestore sera respecté
    ...
    // 4. Sauvegarder dans Firestore (l'UID du doc = l'UID Firebase Auth)
