...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    try {
      ...
      if (snap.exists() && snap.data().nom) {
        // Profil complet → utilisateur inscrit
        currentUser = { uid: user.uid, ...snap.data() };
      } else {
        // Pas de profil Firestore = compte fantôme → on le déconnecte
        await signOut(auth);
        return;
      }
    } catch(e) {
      // Erreur réseau → déconnexion par sécurité
      await signOut(auth);
      return;
    }
...
async function loadCatalogue(svcId) {
  ...
  try {
    ...
    fsError = true;
    console.warn('[Catalogue] Erreur Firestore:', e.message);
  }

  // Fallback uniquement si erreur réseau (pas si catalogue Firestore simplement vide)
  if (fsError) articles = DEFAULT_ARTICLES[svcId] || [];
...
// ════════════════════════════════════════
// FEED PHOTOS PUBLICITAIRES (remplace le slider)
// Photos postées par l'admin via la section "Publications Partenaires"
// ════════════════════════════════════════
async function loadPubFeed() {
  const feed = document.getElementById('pub-feed');
  if (!feed) return;
  try {
    const q = query(collection(db,'partenaires'), orderBy('ordre','asc'));
    const snap = await getDocs(q);
    if (snap.empty) { feed.style.display = 'none'; return; }
    const items = [];
    snap.forEach(d => { const p = d.data(); if (p.imageUrl) items.push(p); });
    if (items.length === 0) { feed.style.display = 'none'; return; }
    feed.innerHTML = items.map(p => `
      <div class="pub-item" ${p.lien ? `onclick="window.open('${p.lien}','_blank')" style="cursor:pointer"` : ''}>
        <img src="${p.imageUrl}" alt="${p.nom||''}" loading="lazy"
             onerror="this.parentElement.style.display='none'"/>
        ${p.nom ? `<div class="pub-caption">${p.nom}${p.description ? '<br><span style="font-weight:400;font-size:11px;opacity:.85">'+p.description+'</span>' : ''}</div>` : ''}
      </div>`).join('');
    feed.style.display = 'flex';
  } catch(e) {
    feed.style.display = 'none';
    console.warn('[PubFeed]', e.message);
  }
}
loadPubFeed();
