...
// IDs des bandeaux standards avec leur attribut data-service correspondant dans le HTML
...
const PROMO_LABELS = {
  ...
  collaborateur:'COLLABORATEUR', nouveaute:'NOUVEAUTÉ',
  promotion:'PROMOTION', evenement:'ÉVÉNEMENT'
};
...
async function loadPartnerSlides() {
  ...
  try {
    ...
    snap.forEach(d => {
      ...
      // ── Bandeaux standards : juste appliquer masquage / overrides ──
      if (STD_BANNER_IDS.has(p.id)) {
        // Extraire le service depuis l'id : "std-delivery" → "delivery"
        ...
        if (stdCard) {
          if (p.actif === false) {
            ...
            const cards = Array.from(track.querySelectorAll('.adcard'));
            const idx = cards.indexOf(stdCard);
            const dots = Array.from(dotsBox.querySelectorAll('.adband-dot'));
            if (dots[idx]) dots[idx].remove();
            stdCard.remove();
          } else {
            // Mettre à jour les textes si l'admin les a modifiés
            if (p.nom) {
              const t = stdCard.querySelector('.adcard-title');
              if (t) t.textContent = p.nom;
            }
            if (p.badge) {
              const b = stdCard.querySelector('.adcard-badge');
              if (b) b.textContent = p.badge;
            }
            if (p.description) {
              const s = stdCard.querySelector('.adcard-sub');
              if (s) s.textContent = p.description;
            }
            if (p.imageUrl) {
              const imgZone = stdCard.querySelector('.adcard-img');
              if (imgZone) {
        ...
        return; // ne pas créer de nouvelle carte
      }

      // ── Bandeaux partenaires purs (Firestore uniquement) ──
      if (p.actif === false) return;           // masqué par l'admin
      if (!p.nom && !p.imageUrl) return;       // vide, on ignore
      ...
      card.innerHTML = `
        ...
        <div class="adcard-body">
          <div class="adcard-promo">${PROMO_LABELS[p.promo]||'PARTENAIRE OFFICIEL'}</div>
...
(function initAdbandLoop() {
  ...
  const INTERVAL = 2400; // ms entre chaque slide
