...
// ════════════════════════════════════════
// BANDEAU PUBLICITAIRE — DÉFILEMENT EN BOUCLE INFINIE (sans retour visible)
// ════════════════════════════════════════
(function initAdbandLoop() {
  ...
  let realCount = 0;   // nombre de vraies cartes
  let currentIdx = 0;  // index dans les vraies cartes (0 à realCount-1)
  let autoTimer = null;
  let isUserScrolling = false;
  let scrollTimeout = null;
  let isJumping = false; // flag pour ignorer l'event scroll pendant le saut silencieux

  function getRealCards() {
    return track.querySelectorAll('.adcard:not(.adcard-clone)');
  }
  ...
  // Injecter les clones (copie de la dernière carte au début, copie de la première à la fin)
  function setupClones() {
    const real = Array.from(getRealCards());
    realCount = real.length;
    if (realCount < 2) return;

    // Clone de la dernière → insérer au début
    const cloneFirst = real[realCount - 1].cloneNode(true);
    cloneFirst.classList.add('adcard-clone');
    track.insertBefore(cloneFirst, real[0]);

    // Clone de la première → insérer à la fin
    const cloneLast = real[0].cloneNode(true);
    cloneLast.classList.add('adcard-clone');
    track.appendChild(cloneLast);
  }

  // Positionner sur une carte (index dans getAllCards)
  function jumpTo(allIdx, smooth) {
    const cards = Array.from(getAllCards());
    if (!cards.length) return;
    const card = cards[allIdx];
    track.scrollTo({ left: card.offsetLeft - track.offsetLeft, behavior: smooth ? 'smooth' : 'instant' });
  }

  // Aller à la vraie carte d'index realIdx (avec animation)
  function goToReal(realIdx) {
    currentIdx = ((realIdx % realCount) + realCount) % realCount;
    // Dans getAllCards(), les vraies cartes commencent à l'index 1 (après le clone initial)
    jumpTo(currentIdx + 1, true);
    updateDot(currentIdx);
  }

  function nextCard() {
    goToReal(currentIdx + 1);
  }
  ...
  // Gérer le saut silencieux quand on arrive sur un clone
  track.addEventListener('scrollend', () => {
    ...
    const total = cards.length; // realCount + 2 clones

    if (closest === 0) {
      // On est sur le clone de la dernière → sauter silencieusement à la vraie dernière
      isJumping = true;
      jumpTo(realCount, false); // index realCount dans getAllCards = dernière vraie carte
      currentIdx = realCount - 1;
      updateDot(currentIdx);
      requestAnimationFrame(() => { isJumping = false; });
    } else if (closest === total - 1) {
      // On est sur le clone de la première → sauter silencieusement à la vraie première
      isJumping = true;
      jumpTo(1, false);
      currentIdx = 0;
      updateDot(currentIdx);
  ...
  // Démarrer après que Firestore ait injecté les cartes
  setTimeout(() => {
    setupClones();
    if (realCount < 2) {
      updateDot(0);
    ...
    // Se positionner sur la première vraie carte (index 1 dans getAllCards)
    jumpTo(1, false);
    updateDot(0);
    startLoop();
  }, 1500);
