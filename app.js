...
// ── Connexion anonyme Firebase Auth (nécessaire pour les règles Firestore) ──
// Toujours s'assurer qu'un token Firebase Auth est présent avant toute requête
...
async function doLogin() {
  ...
  try {
    // S'assurer qu'un token Firebase Auth est présent (requis par les règles Firestore)
...
async function doSignup() {
  ...
  try {
    ...
    // Utiliser l'UID anonyme Firebase (auth.currentUser est garanti après ensureFirebaseAuth)
    const newUid = auth.currentUser ? auth.currentUser.uid : crypto.randomUUID();
