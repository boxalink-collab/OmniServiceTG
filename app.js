...
(function initAdBand() {
  ...
  const dotsEl  = document.getElementById('adband-dots');
  if (!track) return;

  let currentIdx = 0;
  let autoTimer  = null;

  // ── Calculer l'index visible selon la position de scroll ──
  function getVisibleIdx() {
    const cards = track.querySelectorAll('.adcard');
  ...
  function updateDots(idx) {
    dotsEl?.querySelectorAll('.adband-dot').forEach((d, i) => {
  ...
  // ── Avance automatique toutes les 4 s ──
  function startAuto() {
  ...
  // ── Sync dots au scroll manuel ──
  track.addEventListener('scroll', () => {
    updateDots(getVisibleIdx());
  }, { passive: true });

  // ── Pause auto sur interaction, reprend après 6 s ──
  ...
  // ── Ajouter les publicités partenaires depuis Firestore ──
    ...
    try {
      ...
      snap.forEach(p => {
        ...
        if (d.lien) {
          ...
          card.addEventListener('click', () => window.open(d.lien, '_blank'));
        }

        // Zone image : photo réelle si disponible, sinon dégradé + emoji
        const bgStyle = d.couleur
          ? `background:${d.couleur}`
          : `background:linear-gradient(135deg,#1E6FBE,#155A9C)`;

        const imgContent = d.imageUrl
          ? `<img src="${d.imageUrl}" alt="${d.nom||''}" loading="lazy"/>`
          : `<span class="adcard-emoji-bg">${d.emoji||'⭐'}</span>`;

        const ctaHtml = d.lien ? '<div class="adcard-cta">Découvrir →</div>' : '';
        card.innerHTML = `
          <div class="adcard-img" style="${bgStyle}">
            <div class="adcard-gradient"></div>
            ${imgContent}
          </div>
          <div class="adcard-badge">${d.badge||d.nom||'Partenaire'}</div>
          <div class="adcard-body">
            <div class="adcard-promo">${d.promo||'PARTENAIRE OFFICIEL'}</div>
            <div class="adcard-title">${(d.nom||'').replace(/\\n/g,'\n')}</div>
            <div class="adcard-sub">${d.description||''}</div>
            ${ctaHtml}
          </div>`;
      ...
      // Relancer l'auto avec les nouvelles cartes
      clearInterval(autoTimer);
      startAuto();
    } catch(e) {
      console.log('[AdBand] Partenaires non disponibles :', e.message);
    }
  }
