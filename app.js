/* ══════════════════════════════════════════
   OmniService TG — app.js v3.0
   + Splash Screen + PWA + Commandes stables
   ══════════════════════════════════════════ */
...
// Masquer le splash après 2.8s (loader animé dure ~2.8s au total)
...
window.addEventListener('beforeinstallprompt', (e) => {
  e.preventDefault();
  deferredPrompt = e;
  // Afficher la bannière d'installation après le splash
  setTimeout(() => {
...
document.getElementById('pwa-install-btn')?.addEventListener('click', async () => {
  ...
  if (result.outcome === 'accepted') {
    showToast('✅ OmniService TG installé !', '#2E7D32');
  }
});
...
// ════════════════════════════════════════
// RECHERCHE COMMANDES — VERSION STABLE
// ════════════════════════════════════════
async function searchOrders() {
  ...
  if (!ph) {
    out.innerHTML = `
      <div class="orders-empty">
        ...
        <div class="orders-empty-title">Numéro requis</div>
        <div class="orders-empty-sub">Veuillez entrer votre numéro de téléphone pour rechercher vos commandes</div>
      </div>`;
    return;
  }
  ...
  out.innerHTML = `
    <div class="orders-empty">
      ...
      <div class="orders-empty-title">Recherche en cours...</div>
    </div>`;
  ...
  try {
    ...
    if (snap.empty) {
      out.innerHTML = `
        <div class="orders-empty">
          <div class="orders-empty-ico">🔍</div>
          <div class="orders-empty-title">Aucune commande trouvée</div>
          <div class="orders-empty-sub">Aucune commande associée à ce numéro.<br/>Vérifiez que le numéro est identique à celui utilisé lors de votre commande.</div>
        </div>`;
      return;
    }
    ...
    let h = `<div style="font-size:12px;color:var(--light);margin-bottom:12px">${snap.size} commande${snap.size>1?'s':''} trouvée${snap.size>1?'s':''}</div>`;
    ...
    out.innerHTML = `
      <div class="orders-empty">
        ...
        <div class="orders-empty-sub">Impossible de charger vos commandes.<br/>Vérifiez votre connexion internet et réessayez.</div>
      </div>`;
  }
}
...
function saveProfile() {
  const n = document.getElementById('p-name-inp').value.trim() || 'Mon Profil';
  const p = document.getElementById('p-phone-inp').value.trim() || 'Ajoutez vos informations';
  ...
  showToast('✅ Profil enregistré !', '#2E7D32');
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (n) {
    document.getElementById('prof-name-disp').textContent = n;
    document.getElementById('p-name-inp').value = n;
  }
  if (p) {
    document.getElementById('prof-phone-disp').textContent = p;
    document.getElementById('p-phone-inp').value = p;
    if (p !== 'Ajoutez vos informations') {
