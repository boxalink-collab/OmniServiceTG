...
// ════════════════════════════════════════
// CHARGEMENT PARTENAIRES → BANDEAU ADCARD
// ════════════════════════════════════════
  try {
    ...
    snap.forEach(d => {
      ...
      // ── Créer l'adcard partenaire identique au style client ──
      ...
      card.innerHTML = `
        <div class="adcard-img" style="background:linear-gradient(135deg,#1A1A2E 0%,#1E6FBE 100%)">
          ...
          ${p.imageUrl
            ? `<img src="${p.imageUrl}" alt="${p.nom||'Partenaire'}"
      ...
      // Injecter dans le bandeau (après les cartes statiques)
      track.appendChild(card);

      // Ajouter un dot correspondant
...
(function initAdbandLoop() {
  ...
  function getCards() { return track.querySelectorAll('.adcard'); }
  function getDots()  { return dotsContainer ? dotsContainer.querySelectorAll('.adband-dot') : []; }

  function updateDot(idx) {
    getDots().forEach((d, i) => d.classList.toggle('on', i === idx));
  }
  ...
  function startLoop() {
    ...
    autoTimer = setInterval(() => {
      if (!isUserScrolling) scrollToCard(currentIdx + 1);
    }, 500);
  }
  ...
  // Laisser Firestore injecter les cartes partenaires avant de démarrer
  setTimeout(() => { updateDot(0); startLoop(); }, 1500);
})();
